...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    auto aiToGLM = [](aiColor3D& col) -> glm::vec3 {
        return glm::vec3(col.r, col.g, col.b);
    };
    SolidSurface base;
    base.color_ambient = aiToGLM(Ka);
    base.color_diffuse = aiToGLM(Kd);
    base.color_specular = aiToGLM(Ks);
    base.shininess = shininess;
    base.opacity = opacity;
    
    // return a mesh object created from the extracted mesh data
    return Mesh(vertices, indices, textures, base);
}
...
unsigned int TextureFromFile(const char *path, const string &directory, bool gamma)
{
    ...
    if (data)
    {
        ...
        if (nrComponents == 1)
            ...
            cout << "Unknown color format in texture: " << path << endl;
            format = GL_NONE;
        }
